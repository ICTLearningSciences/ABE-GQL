...
describe("delete panelist", () => {
  ...
  it("can delete a panelist", async () => {
    ...
    const response = await request(app)
      ...
      .send({
        query: `mutation DeletePanelist($panelistClientId: String!) {
        ...
        variables: {
          panelistClientId: "test-panelist-delete-1",
        },
      });
    expect(response.status).to.equal(200);
    expect(response.body.data.deletePanelist.clientId).to.equal(
  ...
  it("rejects unauthenticated requests", async () => {
    const response = await request(app)
      ...
      .send({
        ...
        variables: {
          panelistClientId: "test-panelist-delete-2",
        },
      });
    expect(response.status).to.equal(200);

...
*/
import { GraphQLObjectType, GraphQLNonNull, GraphQLEnumType, GraphQLID } from "graphql";
import { UserRole } from "../models/User";
import SectionModel, { Section, SectionType, SectionInputType } from "../models/Section";
...
export const addOrUpdateSection = {
  ...
  resolve: async (
    ...
    const instructorData = await InstructorDataModel.findOne({ userId: context.userId });
    ...
    if (course.instructorId !== context.userId && context.userRole !== UserRole.ADMIN) {
      throw new Error("unauthorized: only course instructor or admin can modify sections");
    }
    ...
    if (!args.sectionData || !args.sectionData._id) {
      throw new Error("section data with _id is required for MODIFY and DELETE actions");
    }
